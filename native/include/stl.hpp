 ...
 * Seasonal Patterns. arXiv:2107.13462 [stat.AP].
...
template <typename T>
bool est(const T *y, size_t n, size_t len, int ideg, T xs, T *ys, size_t nleft,
         size_t nright, T *w, bool userw, const T *rw) {
  auto range = ((T)n) - 1.0;
...
template <typename T>
void ess(const T *y, size_t n, size_t len, int ideg, size_t njump, bool userw,
         const T *rw, T *ys, T *res) {
  if (n < 2) {
    ys[0] = y[0];
    return;
  }
  ...
  if (len >= n) {
    ...
    for (size_t i = 1; i <= n; i += newnj) {
      auto ok =
          est(y, n, len, ideg, (T)i, &ys[i - 1], nleft, nright, res, userw, rw);
      if (!ok) {
        ys[i - 1] = y[i - 1];
      }
    }
  ...
  if (newnj != 1) {
    ...
    if (k != n) {
      auto ok =
          est(y, n, len, ideg, (T)n, &ys[n - 1], nleft, nright, res, userw, rw);
      if (!ok) {
        ys[n - 1] = y[n - 1];
      }
      if (k != n - 1) {
...
template <typename T> void ma(const T *x, size_t n, size_t len, T *ave) {
  auto newn = n - len + 1;
  double flen = (T)len;
  double v = 0.0;
...
template <typename T> void rwts(const T *y, size_t n, const T *fit, T *rw) {
...
template <typename T>
void ss(const T *y, size_t n, size_t np, size_t ns, int isdeg, size_t nsjump,
        bool userw, T *rw, T *season, T *work1, T *work2, T *work3, T *work4) {
  for (size_t j = 1; j <= np; j++) {
    ...
    auto ok =
        est(work1, k, ns, isdeg, xs, &work2[0], 1, nright, work4, userw, work3);
    if (!ok) {
      work2[0] = work2[1];
    }
    xs = k + 1;
    size_t nleft = (size_t)std::max(1, (int)k - (int)ns + 1);
    ok = est(work1, k, ns, isdeg, xs, &work2[k + 1], nleft, k, work4, userw,
             work3);
    if (!ok) {
      work2[k + 1] = work2[k];
    }
...
template <typename T>
void onestp(const T *y, size_t n, size_t np, size_t ns, size_t nt, size_t nl,
            int isdeg, int itdeg, int ildeg, size_t nsjump, size_t ntjump,
            size_t nljump, size_t ni, bool userw, T *rw, T *season, T *trend,
            T *work1, T *work2, T *work3, T *work4, T *work5) {
  for (size_t j = 0; j < ni; j++) {
    for (size_t i = 0; i < n; i++) {
      work1[i] = y[i] - trend[i];
    }

    ss(work1, n, np, ns, isdeg, nsjump, userw, rw, work2, work3, work4, work5,
       season);
...
template <typename T>
void stl(const T *y, size_t n, size_t np, size_t ns, size_t nt, size_t nl,
         int isdeg, int itdeg, int ildeg, size_t nsjump, size_t ntjump,
         size_t nljump, size_t ni, size_t no, T *rw, T *season, T *trend) {
  if (ns < 3) {
  ...
  while (true) {
    onestp(y, n, np, ns, nt, nl, isdeg, itdeg, ildeg, nsjump, ntjump, nljump,
           ni, userw, rw, season, trend, work1.data(), work2.data(),
...
template <typename T> double var(const std::vector<T> &series) {
  auto mean =
...
template <typename T>
float strength(const std::vector<T> &component,
               const std::vector<T> &remainder) {
  std::vector<T> sr;
...
/// A STL result.
template <typename T = float> class StlResult {
public:
  ...
  inline float trend_strength() const { return strength(trend, remainder); }
};
...
StlParams params() { return StlParams(); }

template <typename T>
StlResult<T> StlParams::fit(const T *series, size_t series_size,
                            size_t period) const {
  auto y = series;
  auto np = period;
  auto n = series_size;
  ...
  auto res = StlResult<T>{std::vector<T>(n), std::vector<T>(n),
                          std::vector<T>(), std::vector<T>(n)};
  ...
  stl(y, n, newnp, newns, nt, nl, isdeg, itdeg, ildeg, nsjump, ntjump, nljump,
      ni, no, res.weights.data(), res.seasonal.data(), res.trend.data());
...
/// A MSTL result.
template <typename T = float> class MstlResult {
public:
  /// Returns the seasonal component.
...
public:
  ...
  /// Decomposes a time series from an array.
  template <typename T>
  MstlResult<T> fit(const T *series, size_t series_size, const size_t *periods,
                    size_t periods_size) const;

  /// Decomposes a time series from a vector.
  template <typename T>
  MstlResult<T> fit(const std::vector<T> &series,
                    const std::vector<size_t> &periods) const;

#if __cplusplus >= 202002L
  /// Decomposes a time series from a span.
  template <typename T>
  MstlResult<T> fit(std::span<const T> series,
                    std::span<const size_t> periods) const;
#endif
};

/// Creates a new set of MSTL parameters.
MstlParams mstl_params() { return MstlParams(); }
...
mstl(const T *x, size_t k, const size_t *seas_ids, size_t seas_size,
     size_t iterate, std::optional<float> lambda,
  ...
  auto deseas = lambda.has_value() ? box_cox(x, k, lambda.value())
                                   : std::vector<T>(x, x + k);
    ...
    for (size_t j = 0; j < iterate; j++) {
      for (size_t i = 0; i < indices.size(); i++) {
        ...
        if (swin) {
          ...
          StlParams clone = stl_params;
          fit =
              clone.seasonal_length(7 + 4 * (i + 1)).fit(deseas, seas_ids[idx]);
        }
...
template <typename T>
MstlResult<T> MstlParams::fit(const T *series, size_t series_size,
  ...
  auto [trend, remainder, seasonal] =
      mstl(series, series_size, periods, periods_size, iterate_, lambda_, swin_,
           stl_params_);

  return MstlResult<T>{seasonal, trend, remainder};
}

template <typename T>
MstlResult<T> MstlParams::fit(const std::vector<T> &series,
                              const std::vector<size_t> &periods) const {
  return MstlParams::fit(series.data(), series.size(), periods.data(),
                         periods.size());
}

#if __cplusplus >= 202002L
template <typename T>
MstlResult<T> MstlParams::fit(std::span<const T> series,
                              std::span<const size_t> periods) const {
  return MstlParams::fit(series.data(), series.size(), periods.data(),
                         periods.size());
}
